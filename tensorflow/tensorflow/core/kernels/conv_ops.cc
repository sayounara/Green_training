...
#include "matconv_fpga/Conv2DFPGA.h"
#include <string.h>
#include <map>
#include <vector>
...
static Conv2DFPGAInterface conv2d_fpga_;
template <typename Device, typename T>
class Conv2DOp : public BinaryOp<T> {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    conv2d_fpga_.Compute(input.flat<T>().data(), filter.flat<T>().data(), output->flat<T>().data(), input_rows, input_cols, filter_rows, filter_cols);
  }
