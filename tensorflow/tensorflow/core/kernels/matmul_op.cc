...
#include "matconv_fpga/MatConvFPGA.h"
...
static MatConvFPGAInterface matconv_fpga_;
...
class MatMulOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
	...
	unsigned a_height=a.dim_size(a_dim_remaining);
	unsigned a_width=a.dim_size(dim_pair[0].first);
	unsigned b_width=b.dim_size(b_dim_remaining);
	/*#ifdef PRINTINFO2
	std::cout<<"Matrix A: "<<a_height<<" x "<<a_width<<std::endl;
	std::cout<<"Matrix B: "<<a_width<<" x "<<b_width<<std::endl;
	...
	//if(a_height%BLOCK_SIZE==0 && a_width%BLOCK_SIZE==0 && b_width%BLOCK_SIZE==0) {
       ...
       // ctx, a, b, dim_pair, &algorithms_, use_autotune_, out);
		const void *a_temp=a.flat<T>().data();
		const void *b_temp=b.flat<T>().data();
		void *out_temp=out->flat<T>().data();
		// const T *af=a.flat<T>().data();
		// std::cout<<"a_temp[0]="<<af[0]<<"\n";//", b_temp[0]="<<b_temp[]<<", out_temp[0]="<<out_temp[0]<<"\n";
		matconv_fpga_.Compute(a_temp, b_temp, out_temp, a_height, a_width, b_width);
	//}
	//else
    //	LaunchMatMul<Device, T, USE_CUBLAS>::launch(
   //     ctx, a, b, dim_pair, &algorithms_, use_autotune_, out);
