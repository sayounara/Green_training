...
public:
	...
	AddNFPGAImp() {
		...
		std::string filepath="../fpga_bitstreams/AddN_fpga.aocx";//+binary_file;
  		// printf("filepath: %s\n", filepath.c_str());
  		// printf("Using AOCX: %s\n", binary_file.c_str());
  ...
  // 		kernel.reset(num_devices_);
    ...
    void Compute(const void *a, const void *b, void *c)  {
    	unsigned input_rows=in_rows;

		const float *input_a=(const float*)a;
		const float *input_b=(const float*)b;
		...
		if (BLOCK_SIZE>=256)
		{
			kernel = clCreateKernel(program, "vectadd", &status);
		}
...
AddNFPGAImpInterface::AddNFPGAImpInterface() {
	_AddNFPGAImp = new AddNFPGAImp();
} 
...
void AddNFPGAImpInterface::Compute(const void *a, const void *b, void *c) {
	_AddNFPGAImp->Compute(in, filt, out, in_rows, in_cols, filt_rows, filt_cols);
} 
