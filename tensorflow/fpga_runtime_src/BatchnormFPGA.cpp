...
public:
	...
	BatchnormFPGAImp() {
		...
		std::string filepath="../fpga_bitsreams/Batchnorm_fpga.aocx";//+binary_file;
  		// printf("filepath: %s\n", filepath.c_str());
  		// printf("Using AOCX: %s\n", binary_file.c_str());
...
// 		return;
		...
		float *pMaps_=(float*)pMaps;
		float *pScale_=(float*)pScale;
		float *pOffset_=(float *)pOffset;
	    	float *pOutput_=(float *)pOutput;
		...
		if (BLOCK_SIZE>=256)
		{
			kernel = clCreateKernel(program, "batch_norm", &status);
		}
...
BatchnormFPGAImpInterface::BatchnormFPGAImpInterface() {
	_BatchnormFPGAImp = new BatchnormFPGAImp();
} 
...
void BatchnormFPGAImpInterface::Compute(void *pMaps, void *pScale, void *pOffset, void *pOutput) {
	_BatchnormFPGAImp->Compute(in, filt, out, in_rows, in_cols, filt_rows, filt_cols);
} 
