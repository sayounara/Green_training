...
public:
	...
	Maxpool2DFPGAImp() {
		...
		std::string filepath="../fpga_bitsreams/Maxpool2D_fpga.aocx";//+binary_file;
  		// printf("filepath: %s\n", filepath.c_str());
  		// printf("Using AOCX: %s\n", binary_file.c_str());
  ...
  // 		kernel.reset(num_devices_);
    ...
    void Compute(const int input_size, const int output_size, void *input_im, void *output_im)  {
		...
		if (BLOCK_SIZE>=256)
		{
			kernel = clCreateKernel(program, "maxpool2d", &status);
		}
...
Maxpool2DFPGAImpInterface::Maxpool2DFPGAImpInterface() {
	_Maxpool2DFPGAImp = new Maxpool2DFPGAImp();
} 
...
void Maxpool2DFPGAImpInterface::Compute(const int input_size, const int output_size, void *input_im, void *output_im) {
	_Maxpool2DFPGAImp->Compute(const int input_size, const int output_size, void *input_im, void *output_im);
} 
