...
public:
	...
	SoftmaxFPGAImp() {
		...
		std::string filepath="/home/jliu/hexin_workspace/fpga_tf/opencl_kernel_aocx/Softmax_fpga.aocx";//+binary_file;
  		// printf("filepath: %s\n", filepath.c_str());
  		// printf("Using AOCX: %s\n", binary_file.c_str());
  ...
  // 		kernel.reset(num_devices_);
    ...
    void Compute(const void *in)  {
		...
		if (BLOCK_SIZE>=256)
		{
			kernel = clCreateKernel(program, "softmax", &status);
		}
...
SoftmaxFPGAImpInterface::SoftmaxFPGAImpInterface() {
	_SoftmaxFPGAImp = new SoftmaxFPGAImp();
} 
...
void SoftmaxFPGAImpInterface::Compute(const void *in) {
	_SoftmaxFPGAImp->Compute(in, filt, out, in_rows, in_cols, filt_rows, filt_cols);
} 
