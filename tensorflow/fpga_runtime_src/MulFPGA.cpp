...
public:
	...
	MulFPGAImp() {
		...
		std::string filepath="/home/jliu/hexin_workspace/fpga_tf/opencl_kernel_aocx/Mul_fpga.aocx";//+binary_file;
  		// printf("filepath: %s\n", filepath.c_str());
  		// printf("Using AOCX: %s\n", binary_file.c_str());
...
MulFPGAImpInterface::MulFPGAImpInterface() {
	_MulFPGAImp = new MulFPGAImp();
} 
...
void MulFPGAImpInterface::Compute(const void *a, const void *b, void *c) {
	_MulFPGAImp->Compute(in, filt, out, in_rows, in_cols, filt_rows, filt_cols);
} 
