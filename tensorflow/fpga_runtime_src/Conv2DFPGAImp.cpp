...
public:
	...
	Conv2DFPGAImp() {
		...
		std::string filepath="/home/jliu/hexin_workspace/fpga_tf/opencl_kernel_aocx/conv2d_fpga.aocx";//+binary_file;
  		// printf("filepath: %s\n", filepath.c_str());
  		// printf("Using AOCX: %s\n", binary_file.c_str());
  ...
  // 		kernel.reset(num_devices_);
    ...
    void Compute(const void *in, const void *filt, void *out, unsigned in_rows, unsigned in_cols, unsigned filt_rows, unsigned filt_cols)  {
		...
		if (BLOCK_SIZE>=256)
		{
			kernel = clCreateKernel(program, "convolute", &status);
		}
...
Conv2DFPGAImpInterface::Conv2DFPGAImpInterface() {
	_Conv2DFPGAImp = new Conv2DFPGAImp();
} 
...
void Conv2DFPGAImpInterface::Compute(const void *in, const void *filt, void *out, unsigned in_rows, unsigned in_cols, unsigned filt_rows, unsigned filt_cols) {
	_Conv2DFPGAImp->Compute(in, filt, out, in_rows, in_cols, filt_rows, filt_cols);
} 
